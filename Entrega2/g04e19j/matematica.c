...
double get_normainfinito_matd(const double** mat, size_t fil, size_t col){
	size_t i,j;
	double sum_fil;
	double normainfinito;

	for(i = 0, normainfinito = get_abs_d(mat[0][0]); i < fil; i++){
		for(j = 0, sum_fil = 0 ; j < col; j++){
			sum_fil += get_abs_d(mat[i][j]);
		}
		if(normainfinito < sum_fil)
			normainfinito = sum_fil;
	}
