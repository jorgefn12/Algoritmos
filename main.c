...
int main(int argc, char** argv) {
    ...
    if (status != ST_OK)
    	...
    	if (status != ST_OK)
            imprimir_errores(status);
          /* AGREGAR ELSE PARA IMPRIMIR DUMP */
            ...
            case LEER:
                for(i = 0; i < MAX_INGRESOS; i++){
                    ...
                    if(fgets(aux, MAX_STR, stdin) == NULL){
                        fprintf(stderr, "%s\n", "Ingrese nuevamente, no se leyò bien");
                        continue;
                    }
                    if((temp = strtol(aux, &p, 10)) > MAX_PALABRA || temp < MIN_PALABRA){
                        fprintf(stderr, "%s\n", "Ingrese nuevamente, el rango es invalido [-9999;+9999]");
                        continue;
                    }
                    if(*p != '\n' && *p != '\0' && *p != EOF){
                        fprintf(stderr, "%s\n", "Ingrese nuevamente, el numero no es vàlido");
                        continue;
                    }
                    break;
                }
